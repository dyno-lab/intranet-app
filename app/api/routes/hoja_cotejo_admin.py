...
def _build_excel_bytes(context: dict) -> bytes:
    ...
    row_index = 5
    headers = ["Programa", "Actividad", "Descripción", "Realizadas", "Duplicados / Personas impactadas", "Cumplimiento", "% Cumplimiento", "Sí", "No"]
    ...
    for program in context.get("program_blocks", []):
        for item in program.get("rows", []):
            values = [program["program_display_name"], item["activity_code"], item["activity_description"], item["activities_count"], item["duplicados"], item["goal_summary"], f"{item['percent']}%" if item["percent"] is not None else "N/A", "X" if item["met"] else "", "" if item["met"] else "X"]
            for col, value in enumerate(values, start=1):
                cell = ws.cell(row=row_index, column=col, value=value)
                cell.border = border
                cell.alignment = left if col <= 3 or col == 6 else center
            row_index += 1
    for col, width in {"A": 22, "B": 16, "C": 54, "D": 14, "E": 22, "F": 28, "G": 16, "H": 8, "I": 8}.items():
