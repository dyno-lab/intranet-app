...
def resolve_report_scope(current_user: User, employee_id: int | None, db: Session):
    """Resolve the legacy employee_id query value as a residential scope."""
...
def query_visit_sessions(
    ...
    *,
    ...
    if not is_global and residential_id:
...
def calculate_visits_rows_and_summary(
    ...
    *,
    ...
    for session_id, employee_id_value, employee_name, hours, residential_id, residential_name in session_rows:
        residential_label = residential_name or "Sin residencial" if is_global else ""
        summary_key = (employee_id_value, residential_id if is_global else None)
    ...
    rows = sorted(
        ...
        key=lambda row: (row["residential_name"], row["employee_name"]),
    )
    for row in rows:
...
def get_visit_report(
    ...
    *,
    ...
    stmt = select(VisitReport).where(
        ...
        VisitReport.residential_id == residential_id,
    )
    ...
    *,
        ...
        if not period["is_custom"] and period["month"] and period["year"]:
            ...
            if report_ids:
                report_residential_map = {
                    report.report_id: user_residential_map.get(report.residential_id, "Sin residencial")
    ...
    if mapped_activity_ids:
        session_rows = query_visit_sessions(
            ...
            apply_period_filter,
            is_global=is_global,
            residential_id=selected_user.residential_id if selected_user else None,
        )
