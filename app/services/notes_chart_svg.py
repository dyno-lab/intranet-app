...
def _png_data_uri_from_svg(svg: str) -> str:
    # Mantener como data URI SVG mientras no tengamos rasterización local disponible.
    # Algunos renderers PDF toleran mejor data URIs simples que variantes más complejas.
...
def build_notes_pie_chart_svg(title: str, counts: dict[str, int], width: int = 720, height: int = 360) -> str:
    ...
    if total <= 0:
        parts.append(f'<rect x="{chart_x}" y="{chart_y}" width="{chart_w}" height="{chart_h}" fill="#f8f9fa" stroke="#ced4da" stroke-width="2" rx="10"/>')
        parts.append(f'<text x="{chart_x + chart_w/2:.0f}" y="{chart_y + chart_h/2:.0f}" text-anchor="middle" font-size="20" font-family="Arial" fill="#6c757d">Sin datos</text>')
    else:
        parts.append(f'<rect x="{chart_x}" y="{chart_y}" width="{chart_w}" height="{chart_h}" fill="#ffffff" stroke="#ced4da" stroke-width="1.5" rx="10"/>')
        bar_y = chart_y + 22
        for letter in NOTE_LETTERS:
            value = max(int(counts.get(letter, 0) or 0), 0)
            pct = (value / total) * 100 if total else 0
            bar_width = (value / total) * bar_max_width if total else 0
            parts.append(f'<text x="{chart_x + 8}" y="{bar_y + 11:.2f}" font-size="16" font-family="Arial" font-weight="700" fill="#111">{letter}</text>')
            parts.append(f'<rect x="{chart_x + 34}" y="{bar_y}" width="{bar_max_width}" height="18" fill="#edf2f7" rx="4"/>')
            if bar_width > 0:
                parts.append(f'<rect x="{chart_x + 34}" y="{bar_y}" width="{bar_width:.2f}" height="18" fill="{GRADE_COLORS[letter]}" rx="4"/>')
            parts.append(f'<text x="{chart_x + 34 + bar_max_width + 8}" y="{bar_y + 13:.2f}" font-size="14" font-family="Arial" fill="#333">{pct:.2f}%</text>')
            bar_y += 34
    row = 0
    for letter in NOTE_LETTERS:
        value = max(int(counts.get(letter, 0) or 0), 0)
    ...
    parts.append('</svg>')
    svg = ''.join(parts)
    return _png_data_uri_from_svg(svg)
